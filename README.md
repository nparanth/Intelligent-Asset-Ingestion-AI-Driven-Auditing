...
## Project Overview
The project is a mission-critical ingestion pipeline designed for data ingestion. It automates the auditing of technical assets using a **Functionless Serverless Architecture**. 
...
*   **Object Lock (Governance Mode):** Ensures data immutability for 30 days, meeting strict compliance standards.

### **Phase 2: Resilient Event Buffering**
*   **EventBridge Integration:** Captures `Object Created` events directly from S3.
*   **SQS:** Decouples ingestion from processing. This protects the **Amazon Bedrock** model from being throttled by queuing requests during high-volume data dumps.

### **Phase 3: Functionless Orchestration**
*   **EventBridge Pipes:** A component that polls SQS and triggers the workflow without custom Lambda code.
...
*   **Hardened Global Perimeter:** Integrates AWS WAF in monitor mode with CloudFront Origin Access Control (OAC) to inspect traffic and cryptographically authenticate all requests before they ever reach the secure storage enclave.
...
2. **Deploy via CloudFormation:**
    ...
    aws cloudformation deploy \
     ...
     --parameter-overrides AlertEmail=yourname@example.com  
